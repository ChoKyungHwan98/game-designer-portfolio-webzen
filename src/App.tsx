...
function App() {
        ...
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 inline-flex bg-zinc-100/80 backdrop-blur-md p-1 rounded-full border border-black/5 shadow-inner">
          {(['resume', 'cover-letter'] as const).map((tab) => (
            <button key={tab} onClick={() => handleSetResumeTab(tab)}
              className={`relative px-6 py-1.5 rounded-full text-[13px] font-bold transition-all tracking-tight flex items-center justify-center w-[100px] ${resumeTab === tab ? 'text-[#CC0000]' : 'text-zinc-500 hover:text-[#1A1A1A]'}`}>
              {resumeTab === tab && (
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FDFDFB]">
        <div className="flex flex-col items-center gap-4">
          <div className="w-12 h-12 border-4 border-[#CC0000]/20 border-t-[#CC0000] rounded-full animate-spin"></div>
  ...
  return (
    <>
    <div className="min-h-screen font-sans selection:bg-[#CC0000]/20 text-[#2C2C2C] bg-transparent">
      <BackgroundEffects />
      <Navbar
        setView={changeView}
        currentView={view}
