...
export const Hero = ({ onPortfolioClick, onResumeClick, isEditing, content, setContent }: HeroProps) => (
  <section id="hero" className="relative min-h-screen flex flex-col justify-center px-6 md:px-12 py-[120px] overflow-hidden bg-transparent border-b border-black/10">
    ...
    {/* Top accent line */}
    <div className="absolute top-0 left-0 right-0 h-[2px] bg-linear-to-r from-transparent via-[#CC0000]/30 to-transparent" />
    ...
    <div className="max-w-7xl mx-auto w-full relative z-10 flex flex-col lg:flex-row items-center justify-between gap-8 lg:gap-16 mt-12">
      ...
      >
        ...
        <motion.span
          ...
          className="text-[#CC0000] text-[11px] font-black tracking-[0.25em] uppercase mb-6 block"
        >
        ...
        <div className="flex flex-col sm:flex-row justify-start items-center gap-4 w-full sm:w-auto">
          <motion.button
            ...
            className="px-10 py-5 bg-[#CC0000] text-white font-bold flex items-center justify-center gap-3 hover:bg-[#AA0000] transition-all duration-500 text-sm tracking-widest w-full sm:w-auto rounded-full uppercase shadow-lg shadow-[#CC0000]/20 hover:shadow-xl"
          >
          ...
          <motion.button
            ...
            className="px-10 py-5 bg-white border border-zinc-200 text-zinc-600 font-bold hover:border-[#CC0000]/30 hover:text-[#1A2332] hover:bg-[#CC0000]/5 transition-all duration-500 flex items-center justify-center gap-3 text-sm tracking-widest w-full sm:w-auto rounded-full uppercase shadow-sm hover:shadow-md"
          >
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1, duration: 1 }} className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-4">
      ...
      <div className="w-px h-16 bg-zinc-200 relative overflow-hidden">
        <motion.div animate={{ y: [-64, 64] }} transition={{ repeat: Infinity, duration: 2, ease: 'linear' as const }} className="absolute top-0 left-0 w-full h-1/2 bg-[#CC0000]" />
      </div>
    </motion.div>
  </section>
);
