    ...
    <div className="w-full relative">
      ...
      {data.selfIntroductions ? (
        ...
        <div className="flex items-start gap-8 xl:gap-12">
          ...
          {/* 메인 타임라인 */}
          <div className="relative border-l-[3px] border-[#B91C1C]/15 flex-1 min-w-0">
          {data.selfIntroductions.map((intro, idx) => (
            <React.Fragment key={idx}>
              <article className="relative w-full pl-8 md:pl-16 pb-[80px] md:pb-[120px] scroll-mt-24 md:scroll-mt-[140px]" id={`intro-${idx}`}>
                ...
                <div className="absolute -left-[19px] md:-left-[24px] top-[50px] md:top-[68px] lg:top-[72px] w-9 h-9 md:w-11 md:h-11 bg-white border-[3px] border-[#B91C1C]/30 rounded-full flex items-center justify-center text-[#B91C1C] font-mono font-bold text-xs md:text-sm shadow-md ring-4 ring-white">
                ...
                >
                  ...
                  >
                    ...
                    <motion.div
                      ...
                      className="mt-5 h-px bg-linear-to-r from-[#B91C1C]/30 via-[#B91C1C]/10 to-transparent"
                    />
                  </motion.div>
                  ...
                  <motion.div 
                    ...
                    className="max-w-[780px] text-[#1C1C1C] leading-[1.9] text-[16.5px] md:text-[18px] font-medium tracking-[-0.01em] [&_p]:mb-5 md:[&_p]:mb-7 [&_p]:break-keep [&_strong]:text-[#B91C1C] [&_strong]:font-extrabold [&_strong]:bg-[linear-gradient(to_top,rgba(0,71,187,0.22)_50%,transparent_50%)] [&_strong]:px-[3px] [&_strong]:rounded-sm"
                  >
                    {/* Hook */}
                    ...
                    {(intro.pullQuote || isEditing) && (
                      <div className="my-10 md:my-12">
                        ...
                        <blockquote className="border-l-[3px] border-[#B91C1C]/30 bg-[#F8F9FF]/50 py-5 md:py-6 px-6 md:px-8 font-bold text-[24px] md:text-[27px] leading-[1.6] text-[#333F48] rounded-r-xl tracking-tight">
                      ...
                      <div className="my-10 md:my-12">
                        ...
                        <ul className="grid md:grid-cols-4 gap-3 md:gap-4 pl-0">
                          {(intro.highlights || Array(4).fill({ bold: "", em: "" })).map((hl, hlIdx) => (
                            <li key={hlIdx} className="list-none relative px-4 md:px-5 py-5 md:py-6 bg-[#F8F9FF] border border-[#B91C1C]/15 rounded-2xl shadow-[0_4px_20px_-4px_rgba(0,71,187,0.08)] break-normal md:[&:not(:last-child)]::after:content-[''] md:[&:not(:last-child)]::after:absolute md:[&:not(:last-child)]::after:-right-[14px] md:[&:not(:last-child)]::after:top-1/2 md:[&:not(:last-child)]::after:-translate-y-1/2 md:[&:not(:last-child)]::after:border-t-[2.5px] md:[&:not(:last-child)]::after:border-r-[2.5px] md:[&:not(:last-child)]::after:border-[#B91C1C]/40 md:[&:not(:last-child)]::after:w-[10px] md:[&:not(:last-child)]::after:h-[10px] md:[&:not(:last-child)]::after:rotate-45">
                              <strong className="text-[#B91C1C] font-black text-[15px] md:text-[18px] block mb-2 px-0 bg-none!">
                ...
                {idx < (data.selfIntroductions || []).length - 1 && (
                  <div className="absolute bottom-10 left-8 md:left-16 right-0 h-px bg-linear-to-r from-[#B91C1C]/10 via-[#B91C1C]/5 to-transparent" />
                )}
              </article>
          ...
          >
            <div className="flex flex-col gap-6 border-l-2 border-[#B91C1C]/10 pl-6 py-2">
              <div className="text-xs font-black tracking-[0.2em] text-[#B91C1C]/60 mb-2">INDEX</div>
              {data.selfIntroductions.map((intro, idx) => {
                ...
                return (
                  <a 
                    ...
                    className={`text-[14px] font-medium transition-colors relative group block ${isActive ? 'text-[#B91C1C]' : 'text-zinc-400 hover:text-[#B91C1C]'}`}
                  >
                    <span className={`absolute -left-[29px] top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-[#B91C1C] transition-opacity ${isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}/>
        ...
        <div className="bg-white p-8 md:p-12 rounded-2xl border border-black/5 markdown-body">
          {isEditing ? (
            <textarea className="w-full h-[400px] bg-[#0a0a0a] border border-[#2a2a2a] rounded-lg p-6 text-[#e8e4dc] font-sans text-sm focus:outline-none focus:border-[#B91C1C]"
