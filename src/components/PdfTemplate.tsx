  ...
  <div className="pdf-page bg-white mx-auto flex flex-col font-sans" style={{ width: '210mm', minHeight: '297mm' }}>
    ...
    <header className="flex items-start gap-5 p-6 bg-[#FAFAFA] border-b border-zinc-100">
      ...
      <div className="flex-1 flex flex-col items-start pt-1">
        ...
        <p className="text-[#CC0000] font-black font-mono tracking-[0.4em] text-[9.5px] uppercase mb-3 pb-1 border-b-2 border-[#CC0000]">
          {data.role}
        </p>
        
        <div className="max-w-2xl text-[11px] text-[#2C2C2C] leading-relaxed font-medium [&_strong]:text-[#CC0000] [&_strong]:font-bold break-keep italic opacity-90 mb-3">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-[10px] text-zinc-500 font-bold uppercase tracking-wider">
          <div className="flex items-center gap-1.5">
            <Mail className="w-3 h-3 text-[#CC0000]/70" strokeWidth={2} />
            <span className="lowercase">{data.email}</span>
          </div>
          {data.phone && (
            <div className="flex items-center gap-1.5">
              <Phone className="w-3 h-3 text-[#CC0000]/70" strokeWidth={2} />
              <span>{data.phone}</span>
            </div>
          )}
          {data.birthDate && (
            <div className="flex items-center gap-1.5">
              <Calendar className="w-3 h-3 text-[#CC0000]/70" strokeWidth={2} />
              <span>{data.birthDate}</span>
            </div>
          )}
          {data.address && (
            <div className="flex items-center gap-1.5">
              <MapPin className="w-3 h-3 text-[#CC0000]/70" strokeWidth={2} />
              <span>{data.address}</span>
            </div>
          )}
          {data.military && (
            <div className="flex items-center gap-1.5">
              <Shield className="w-3 h-3 text-[#CC0000]/70" strokeWidth={2} />
      ...
      <aside className="col-span-4 p-6 border-r border-zinc-100 bg-[#FCFCFC] flex flex-col gap-6">
        ...
        <section>
          <h3 className="text-[12px] font-bold mb-3 flex items-center gap-2 text-[#1A1A1A]">
            <GraduationCap className="text-[#CC0000] w-3.5 h-3.5" /> 학력 및 교육
          </h3>
          <div className="space-y-4">
            {data.education.map((edu, idx) => (
              <div key={idx} className="relative pl-3 border-l-2 border-[#CC0000]/20">
                <div className="absolute -left-[5px] top-1 w-2 h-2 rounded-none bg-[#CC0000]/40"></div>
        ...
        <section>
          <h3 className="text-[12px] font-bold mb-2 flex items-center gap-2 text-[#1A1A1A]">
            <Award className="text-[#CC0000] w-3.5 h-3.5" /> 자격증
          </h3>
              ...
              <div key={idx} className="flex items-center justify-between py-1.5 border-b border-zinc-100 last:border-0">
                <div className="flex flex-col gap-0.5">
                  <div className="flex items-center gap-1.5">
                    ...
                    {cert.score && <span className="text-[8px] font-black text-[#CC0000] bg-[#CC0000]/5 px-1 py-0.5 rounded-sm leading-none tabular-nums mt-0.5">{cert.score}</span>}
                  </div>
      ...
      <main className="col-span-8 p-6 bg-white flex flex-col gap-6">
        ...
        <section>
          <h3 className="text-[13px] font-bold mb-4 flex items-center gap-2 text-[#1A1A1A]">
            <Briefcase className="text-[#CC0000] w-4 h-4" /> 프로젝트 경험
          </h3>
          <div className="space-y-6">
            {data.experience.map((exp, idx) => (
              <div key={idx} className="relative pl-5 border-l-2 border-[#CC0000]/10">
                <div className="absolute -left-[6px] top-1.5 w-2.5 h-2.5 rounded-full bg-white border-2 border-[#CC0000] shadow-sm"></div>
                ...
                <div className="text-[10px] text-[#CC0000] font-bold mb-1.5 bg-[#CC0000]/5 inline-block px-2 py-1 rounded-sm border-l-2 border-[#CC0000]">
                ...
                <ul className="text-[9.5px] text-[#4A4A4A] space-y-1.5 list-none leading-relaxed font-medium">
                  {exp.details.map((detail, dIdx) => (
                    <li key={dIdx} className="relative pl-3">
                      <span className="absolute left-0 top-1.5 w-1 h-1 border border-[#CC0000] rounded-full"></span>
                      {detail}
                    </li>
                  ))}
                </ul>
        ...
        {data.tools && data.tools.length > 0 && (
          <section className="pt-4 border-t border-zinc-100 mt-auto">
            <h3 className="text-[13px] font-bold mb-3 flex items-center gap-2 text-[#1A1A1A]">
              <Wrench className="text-[#CC0000] w-4 h-4" /> 기술 역량 및 도구
            </h3>
            <div className="grid grid-cols-2 gap-x-6 gap-y-4">
              {/* Group 1 */}
              <div className="space-y-2">
                <h4 className="text-[7.5px] font-black text-[#CC0000] tracking-[0.4em] uppercase border-b border-[#CC0000]/10 pb-1 mb-1.5">DOCUMENTATION & OFFICE</h4>
              ...
              {/* Group 2 */}
              <div className="space-y-2">
                <h4 className="text-[7.5px] font-black text-[#CC0000] tracking-[0.4em] uppercase border-b border-[#CC0000]/10 pb-1 mb-1.5">CREATIVE & ENGINE</h4>
                <div className="space-y-1.5">
                  ...
                  <h4 className="text-[7.5px] font-black text-[#CC0000] tracking-[0.4em] uppercase border-b border-[#CC0000]/10 pb-1 mb-1.5 mt-3">AI ASSISTANTS</h4>
                  <div className="grid grid-cols-2 gap-1.5">
                    {data.tools.filter(t => ["ChatGPT", "Claude", "Gemini", "Antigravity"].includes(t.name)).map((tool, idx) => (
                      <div key={idx} className="flex items-center gap-1">
                        <div className="w-1 h-1 rounded-full bg-[#CC0000]/30" />
