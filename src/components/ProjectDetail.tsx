...
export const ProjectDetail = ({ project, onBack, isEditing, onSaveContent }: ProjectDetailProps) => {
  ...
  const tabs: { id: TabType; label: string; icon: React.ReactNode; show: boolean; color: string }[] = [
    { id: 'overview', label: '개요', icon: <LayoutGrid className="w-3.5 h-3.5" />, show: true, color: '#B91C1C' },
                ...
                <div className="absolute bottom-10 left-10 right-10 flex justify-between items-end pointer-events-none z-50">
                  <div className="pointer-events-auto flex flex-col items-start gap-3">
                    ...
                    <button
                      ...
                      className="flex items-center gap-3 px-6 py-3.5 bg-[#B91C1C] text-white rounded-2xl shadow-2xl shadow-[#B91C1C]/30 hover:scale-105 active:scale-95 transition-all font-sans font-black text-[11px] uppercase tracking-[0.3em]"
                    >
                  ...
                  <div className="pointer-events-auto flex flex-col items-end gap-4">
                    ...
                    <div className="group relative">
                      <div className="absolute bottom-full right-0 mb-6 w-72 p-6 bg-black/95 backdrop-blur-2xl rounded-4xl text-white text-[12px] leading-relaxed opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none border border-white/10 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5)] translate-y-2 group-hover:translate-y-0">
                        <div className="flex items-center gap-3 mb-4 text-[#B91C1C] font-black tracking-widest uppercase text-[10px]">
                        ...
                        <ul className="space-y-2.5 text-zinc-300 font-medium">
                          ...
                          <li className="flex justify-between items-center bg-white/5 p-2 rounded-lg"><span>Ctrl + 휠</span> <span className="text-[10px] bg-[#B91C1C] px-1.5 py-0.5 rounded text-white">확대/축소</span></li>
                      ...
                      <button className="w-14 h-14 rounded-full bg-white text-[#B91C1C] shadow-2xl flex items-center justify-center hover:scale-110 active:scale-95 transition-all border border-black/5">
                          ...
                          <div className="absolute inset-0 bg-linear-to-t from-zinc-900/40 to-transparent flex flex-col justify-end p-10">
                            {project.simulatorUrl && (
                              <a 
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="w-full h-16 bg-[#B91C1C] text-white rounded-2xl flex items-center justify-center gap-3 hover:bg-[#003799] hover:scale-[1.02] active:scale-95 transition-all shadow-xl shadow-[#B91C1C]/20"
                              >
                  ...
                  <div className="space-y-4">
                    ...
                    {project.externalUrl && (
                      <a href={project.externalUrl} target="_blank" rel="noopener noreferrer"
                        className="flex items-center gap-3 p-4 bg-[#B91C1C] rounded-2xl text-white hover:bg-[#003799] transition-colors group"
                      >
          ...
          >
            <div className="flex items-center justify-between mb-16">
              <div className="flex flex-col">
                <span className="text-[#B91C1C] text-[12px] font-black tracking-[0.5em] uppercase mb-3">Navigation</span>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-7 gap-10">
                {galleryImages.map((img, i) => (
                  <motion.button
                    ...
                    className={`group relative aspect-3/4 rounded-2xl overflow-hidden border-2 transition-all duration-500 hover:scale-110 hover:shadow-[0_40px_80px_rgba(0,0,0,0.6)] ${i === currentPage ? 'border-[#B91C1C] shadow-[0_0_40px_rgba(0,71,187,0.5)]' : 'border-white/5 hover:border-white/20'}`}
                  >
                    <img src={img} alt={`Page ${i + 1}`} className="w-full h-full object-cover" />
                    <div className={`absolute inset-0 transition-opacity duration-500 ${i === currentPage ? 'bg-[#B91C1C]/20' : 'bg-black/60 opacity-0 group-hover:opacity-100'}`} />
