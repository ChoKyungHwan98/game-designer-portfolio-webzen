      ...
      <div className="relative w-full h-full flex flex-col justify-end p-6 lg:p-8">
        ...
        <div className={`relative z-10 transition-all duration-500 ${isActive ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0 lg:opacity-100 lg:translate-y-0'}`}>
          <div className={`flex gap-2 mb-4 ${isActive ? 'flex-wrap items-center' : 'flex-col items-start'}`}>
            ...
            {project.status && (
              <div className={`px-3 py-1.5 text-[11px] font-sans font-bold tracking-tight rounded-md w-fit border backdrop-blur-sm ${project.status === '미출시' ? 'bg-zinc-800/80 text-zinc-300 border-zinc-600' : 'bg-[#B91C1C] text-white border-[#B91C1C] shadow-lg shadow-[#B91C1C]/30'}`}>
          ...
          {isActive && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="mt-4">
              ...
              <button onClick={(e) => { e.stopPropagation(); onProjectClick(project); }}
                className="px-6 py-3 bg-white text-[#2C2C2C] font-bold text-xs tracking-widest hover:bg-[#B91C1C] hover:text-white transition-colors flex items-center gap-2 rounded-full uppercase w-fit">
    ...
    <motion.div whileHover={{ scale: 1.02 }} className="group relative bg-white border border-black/5 rounded-3xl overflow-hidden hover:border-black/10 hover:shadow-xl transition-all duration-500 flex flex-col">
      ...
      <div className="overflow-hidden relative bg-zinc-100 shrink-0 aspect-16/10 border-b border-black/5">
        ...
        <div className="absolute top-4 left-4 flex flex-wrap items-center gap-2">
          ...
          {project.status && (
            <div className={`border rounded-md px-3 py-1.5 text-[11px] font-sans font-bold tracking-tight shadow-sm w-fit backdrop-blur-sm ${project.status === '미출시' ? 'bg-zinc-100/90 text-zinc-500 border-zinc-200' : 'bg-[#B91C1C] text-white border-[#B91C1C] shadow-md shadow-[#B91C1C]/20'}`}>
              <EditableText value={project.status} onSave={(v) => { const p = [...projects]; p[idx].status = v; setProjects(p); }} isEditing={isEditing} />
            </div>
          )}
        </div>
      ...
      <div className="flex-1 flex flex-col justify-between p-8">
        <div>
          <h3 className="text-xl font-bold mb-3 text-[#2C2C2C] group-hover:text-[#B91C1C] transition-colors line-clamp-1">
            <EditableText value={project.title} onSave={(v) => { const p = [...projects]; p[idx].title = v; setProjects(p); }} isEditing={isEditing} />
          </h3>
        ...
        <button onClick={(e) => { e.stopPropagation(); onProjectClick(project); }}
          className="w-full relative overflow-hidden group/btn py-4 bg-white border border-black/10 text-[#2C2C2C] font-bold text-xs tracking-widest transition-all duration-500 flex items-center justify-center gap-2 uppercase rounded-xl hover:shadow-[0_4px_16px_rgba(128,0,32,0.15)] hover:border-[#B91C1C]">
          ...
          <div className="absolute inset-0 bg-[#B91C1C] scale-x-0 origin-left group-hover/btn:scale-x-100 transition-transform duration-500" />
        </button>
      </div>
    </motion.div>
  );
