        ...
        <nav className={`relative pointer-events-auto w-[98%] max-w-[1440px] rounded-full transition-all duration-500 flex items-center justify-between px-6 lg:px-8 py-3 ${navBgClass}`}>
          ...
          {currentView === 'resume' && isEditing && (
            <div className="absolute top-1/2 -right-2 lg:-right-4 transform translate-x-full -translate-y-1/2 pointer-events-auto">
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2.5 rounded-full bg-white shadow-sm border border-black/5 hover:border-[#CC0000]/30 hover:shadow text-zinc-600 hover:text-[#CC0000] text-[13px] font-bold transition-all disabled:opacity-50 group"
              >
          ...
          <div className="flex shrink-0 items-center gap-3 min-w-[200px]">
            {isSubView && onBack ? (
              <button
                onClick={onBack}
                className="flex items-center gap-2.5 px-4 py-2 rounded-full bg-zinc-100 hover:bg-zinc-200 border border-black/6 text-zinc-600 hover:text-[#CC0000] font-bold text-sm transition-all duration-200 group"
              >
                <motion.span
              ...
              <div className="flex items-center gap-3 cursor-pointer group" onClick={(e) => handleLinkClick(e, 'hero-top')}>
                <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-linear-to-br from-[#CC0000] to-[#003080] text-white flex items-center justify-center shadow-lg shadow-[#CC0000]/25 transition-all group-hover:scale-105 group-hover:shadow-[#CC0000]/40 group-hover:shadow-xl">
                  <Home className="w-5 h-5 md:w-6 md:h-6 transition-transform group-hover:-translate-y-0.5" strokeWidth={2} />
                </div>
                <div className="flex flex-col justify-center">
                  <span className="font-display font-bold tracking-tight text-[16px] md:text-[18px] text-[#2C2C2C] group-hover:text-[#CC0000] transition-colors leading-none">조경환</span>
                  <span className="text-[10px] md:text-[11px] font-mono tracking-widest uppercase text-zinc-400 mt-1.5 leading-none hidden sm:block group-hover:text-[#CC0000]/60 transition-colors">게임 기획자 지망생</span>
                </div>
                {isEditing && (
                  <span className="ml-2 px-2 py-1 bg-[#CC0000]/10 border border-[#CC0000]/30 rounded text-[10px] text-[#CC0000] font-bold uppercase tracking-wider">
                    Edit
                  </span>
                )}
              </div>
              ...
              <div className="flex items-center justify-center gap-2">
                {[
                  ...
                  <a
                    ...
                    className={`relative px-5 py-3 rounded-full text-[15px] font-bold transition-all flex items-center gap-2.5 ${activeSection === id ? 'text-[#CC0000] bg-[#CC0000]/5' : 'text-zinc-500 hover:text-[#2C2C2C] hover:bg-zinc-100'}`}
                  >
                    <span className={`text-[12px] font-mono uppercase tracking-widest transition-colors duration-300 ${activeSection === id ? 'text-[#CC0000]/70' : 'text-zinc-400'}`}>{num}</span>
          ...
          <div className="flex items-center justify-end gap-3 shrink-0 min-w-[200px]">
            ...
            <div className="flex items-center">
              <button
                ...
                title="Admin Mode"
              >
                <Lock className={`w-[18px] h-[18px] ${isEditing ? 'text-[#CC0000]' : 'opacity-80'}`} />
              </button>
            </div>
              ...
              <div className="bg-zinc-50 rounded-2xl p-4 flex flex-col gap-2">
                ...
                {[
                  ...
                  <button key={item.key} onClick={() => { setView(item.key as any); setIsMenuOpen(false); window.scrollTo(0, 0); }}
                    className={`text-left font-bold text-[14px] flex items-center gap-3 p-3 rounded-xl transition-all duration-300 ${currentView === item.key ? 'bg-white text-[#CC0000] shadow-sm' : 'text-zinc-600 hover:bg-[#CC0000] hover:text-white'}`}>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {[
                    ...
                    <a key={id} href={`#${id}`} onClick={(e) => handleLinkClick(e, id)}
                      ...
                      <span className="text-xs font-mono opacity-40 text-zinc-500 group-hover:text-[#CC0000] transition-colors">{num}</span>
