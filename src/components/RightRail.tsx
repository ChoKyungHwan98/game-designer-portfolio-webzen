          ...
          {sections.map(section => {
            ...
            return (
              <button key={section.id} onClick={() => onNavClick(section.id)} className="group relative flex items-center justify-center w-6 h-6" aria-label={`Scroll to ${section.id}`}>
                ...
                <div className={`rounded-full transition-all duration-300 ease-out ${isActive ? 'w-2 h-6 bg-[#B91C1C] shadow-[0_2px_8px_rgba(0,71,187,0.3)]' : 'w-1.5 h-1.5 bg-zinc-300 group-hover:bg-zinc-400 group-hover:scale-125'}`}></div>
                ...
                <div className={`absolute right-10 flex items-center transition-all duration-300 ${isActive ? 'opacity-100 translate-x-0 pointer-events-auto' : 'opacity-0 translate-x-2 pointer-events-none group-hover:opacity-100 group-hover:translate-x-0'}`}>
                  <div className="bg-white border border-black/5 shadow-sm px-3 py-1.5 rounded-xl text-[12px] font-bold text-[#2C2C2C] whitespace-nowrap flex items-center gap-2">
                    <span className="text-[10px] font-mono tracking-widest text-[#B91C1C]/70">{section.label}</span>
      ...
      <AnimatePresence>
        {isVisible && (
          <motion.button initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }}
            onClick={scrollToTop}
            className="fixed bottom-8 right-8 z-50 w-12 h-12 bg-white/90 backdrop-blur-md border border-black/10 rounded-xl flex items-center justify-center text-[#2C2C2C] hover:bg-zinc-50 hover:border-[#B91C1C] transition-all shadow-md print:hidden">
